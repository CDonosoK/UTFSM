...
comandosPrueba = ["SELECT Nombre FROM Estudiantes;",
                ...
                "SELECT Telefono FROM Estudiantes WHERE Nombre = 'Clemente Aguilar'"]#COMANDO CON ERROR.
...
def verificar(comando):
    ...
    select = re.compile(rf'(?P<tipoComando>SELECT )(?P<elemento_1>{valores}(?: *, *{valores})*) FROM (?P<elemento_0>{valores})( INNER JOIN (?P<elemento_2>{valores}))?( WHERE (?P<elemento_3>({valores} = {valores_com}( AND|OR {valores} = {valores_com})?)*))?( ORDER BY (?P<elemento_4>{valores}) (?P<elemento_5>(DESC|ASC)))?;')
...
def select(comando, lista):
    ...
    listaEntrega = []
    condiciones = lista[4]
    queMostrar = lista[2].split(',')
    asterisco = u"*"
    dif = []
    comp = []
    correcto = []
    if lista[5] != None:
        dif.append(comando.split("ORDER BY"))
        instruccion = dif[0][0]
        i = 0
        cont = 0
    ...
    if asterisco in comando:
        ...
        listaEntrega = []
        for dato in range(len(listaFinal[0])):
            l_aux = []
            for j in listaFinal:
                l_aux.append(j[dato])
            listaEntrega.append(l_aux)
        for condicion in listaCondiciones:
            cond = condicion[1]
            for info in listaEntrega:
                if cond in info:
                    print(' '.join(info))
                    hayDatos = True
            if not hayDatos:
...
def iner_join(comando, palabra):
    intermedio = comando.split(u"WHERE")
    print(intermedio)
    pal = u""
    primer = [-1, -1]
    continuar = True
